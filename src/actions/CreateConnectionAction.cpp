...
CreateConnectionAction::CreateConnectionAction(const AudioProcessorGraph::Connection &connection, bool isDefault, ConnectionsState &connections, ProcessorGraph &processorGraph)
    ...
    if (processorGraph.canAddConnection(connection) &&
        (!isDefault || (processorGraph.getProcessorStateForNodeId(connection.source.nodeID)[ProcessorStateIDs::allowDefaultConnections] &&
                        processorGraph.getProcessorStateForNodeId(connection.destination.nodeID)[ProcessorStateIDs::allowDefaultConnections]))) {
...
ValueTree CreateConnectionAction::stateForConnection(const AudioProcessorGraph::Connection &connection, bool isDefault) {
    ...
    source.setProperty(ProcessorStateIDs::nodeId, int(connection.source.nodeID.uid), nullptr);
    ...
    destination.setProperty(ProcessorStateIDs::nodeId, int(connection.destination.nodeID.uid), nullptr);
