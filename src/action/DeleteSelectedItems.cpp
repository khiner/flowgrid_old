...
DeleteSelectedItems::DeleteSelectedItems(Tracks &tracks, Connections &connections, ProcessorGraph &processorGraph) {
    ...
    for (auto *selectedProcessor : tracks.findAllSelectedProcessors()) {
        deleteProcessorActions.add(new DeleteProcessor(selectedProcessor, tracks, connections, processorGraph));
